...
class AttendanceChecker {
    ...
    showScheduleEditModal(schedule) {
        ...
        const skipWeekendsInput = document.getElementById('scheduleEditSkipWeekends');
            ...
            item.innerHTML = `
                <div class="attendance-info">
                    <input type="text" class="attendance-memo-input" value="${memoValue}" 
                           ...
                           onchange="attendanceChecker.updateAttendanceMemo(${log.id}, this.value)">
        ...
        sortedSchedules.forEach(schedule => {
            ...
            item.innerHTML = `
                ...
                <div class="schedule-item-controls">
                    <button class="schedule-edit-btn" onclick="attendanceChecker.editSchedule(${schedule.id})">Edit</button>
                    <button class="schedule-delete-btn" onclick="attendanceChecker.deleteSchedule(${schedule.id})">Delete</button>
                </div>
            `;
    ...
    renderMemoList() {
        ...
        if (filteredMemos.length === 0) {
            ...
            filteredMemos.forEach(memo => {
                ...
                const timestampHtml = memo.editedAt ? 
                    `<div class="memo-timestamp">Created at ${memo.createdAt}<br>Edited at ${memo.editedAt}</div>` :
                    `<div class="memo-timestamp">Created at ${memo.createdAt}</div>`;
